...
######################################################################
startCounter = 0 # use 0 to fly, and 1 to test
######################################################################
...
# to keep account for speed, introduce inverse relationship between size of the object and speed
# find the height of the object and compare that to the size it appears on the screen
# large view = move slower
# what is the height/width of the screen?
# what is ratio of that and the objects on the screen? how to determine how far away?


def empty(a):
    pass

#[[16, 119, 153], [36, 208, 244]]
...
cv2.createTrackbar("SAT Min","HSV",207,255,empty)
cv2.createTrackbar("SAT Max","HSV",249,255,empty)
...
cv2.createTrackbar("Area", "Parameters", 3399, 30000, empty)
        ...
        if area > areaMin:
            ...
            if (cx < int(frameWidth / 2) - deadZone):
                ...
                cv2.rectangle(imgContour, (0, int(frameHeight / 2 - deadZone)),
                              ...
                              cv2.FILLED)
                dir_horizontal = -(1 - area/(width*height)

            else if (cx > int(frameWidth / 2) + deadZone):
            ...
            else if (cy < int(frameHeight / 2) - deadZone):
                cv2.putText(imgContour, " GO UP ", (20, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 3)
                cv2.rectangle(imgContour, (int(frameWidth / 2 - deadZone), 0),
                              (int(frameWidth / 2 + deadZone), int(frameHeight / 2) - deadZone), (0, 0, 255),
                              cv2.FILLED)
                dir_vertical = -(1 - area/(width*height)
...
while True:
    ...
    tello.yaw_velocity = 60*dir_horizontal;
    tello.up_down_velocity = 60*dir_vertical;
